...
uint8_t SensorData[3] = {17,8, 1};
uint8_t data1[2] = {17,10};
uint8_t data2[2] = {8,14};
...
LIN_table_record_t responderMessageTable[LIN_RESPONDER_NUM_MSGS] = {
    ...
    {0x00, /* PID 0x39 */                               //Přijmi data do bufferu
        2, LIN_Message_1_Handler},
    {
        0x01, //PID adresa                                  //Odesli odpoved
        3,    //Pocet prijatych bajtu
    },
        {
        0x02, //PID adresa                                  //Odesli odpoved
        3,    //Pocet prijatych bajtu
    },
};
...
void setLINResponderRXMessage(UART_Regs *uart, uint8_t data, volatile LIN_STATE *gStateMachine){
    ...
    switch (LIN_state) {
        case LIN_RX_STATE_ID:
            ...
            switch (data) {
                case 0x00:   //Pri detekci techto PID zpracuj data a proved callback
                ...
                case 0x01:    //Pri detekci techto PID pouze odesli data
...
void LIN_0_INST_IRQHandler(void)
{
    ...
    switch (DL_UART_Extend_getPendingInterrupt(LIN_0_INST)) {
        ...
        //Detekce začátku Break pole (Break pole = log 0 po dobu ≥13 bitů = start komunikačního rámce)
